...
BOOL removed;
GPUImageiOSBlurFilter *_blurFliter;
...
{
    if (_blurFliter == nil) {
        _blurFliter = [[GPUImageiOSBlurFilter alloc] init];
        _blurFliter.blurRadiusInPixels = 3.0f;
    }
    
    UIImage *image = [self.view.superview convertViewToImage];
    GPUImagePicture *picture = [[GPUImagePicture alloc] initWithImage:image];
    [picture addTarget:_blurFliter];
    [_blurFliter addTarget:_blurView];
    
    [picture processImageWithCompletionHandler:^{
        [_blurFliter removeAllTargets];
    }];
}
