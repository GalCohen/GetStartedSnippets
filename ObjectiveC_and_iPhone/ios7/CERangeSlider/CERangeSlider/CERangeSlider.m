...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    // calculate a delta, which determines the number of pixels the user’s finger travelled. You then convert it
    // into a scaled value delta based on the minimum and maximum values of the control.
    ...
    // 2. update the values
    //  adjust the upper or lower values based on where the user drags the slider to. Note that you’re using a
    // BOUND macro which is a little easier to read than a nested MIN / MAX call.
    ...
    // sets the disabledActions flag inside a CATransaction. This ensures that the changes to the frame for each
    // layer are applied immediately, and not animated. Finally, setLayerFrames is called to move the knob to the correct location.
    [CATransaction begin];
